...
(1) \textbf{The hash-chain time token mechanism} that directly embeds
...
(2) \textbf{The unified construction of THRESHOLD$(k,n)$ gates and version-based revocation}
...
(3) \textbf{The IND-qCPA-T security model} that establishes a quasi-dynamic
...
(4) \textbf{The closed-loop security framework} that combines BLS signatures
...
of Sentinel-CP-ABE with BSW07~\cite{Bethencourt2007},
Guo24~\cite{Guo2024}, and Zhang24~\cite{Zhang2024_TDSC}.
Sentinel-CP-ABE KeyGen is $(2+2|S_U|)E_G$ (where $E_G$ is an
exponentiation in $\mathbb{G}$), approximately $1.7$--$2.0\times$ BSW07's
...
exponentiation in $\mathbb{G}_T$), comparable to BSW07's
$(2+2|S_T|)E_p + |S_T|E_{G_T}$ while adding temporal predicates,
THRESHOLD gates, revocation, and AI-driven anomaly detection---four
features BSW07 does not support.

Guo24 achieves sub-millisecond decryption via pairing-free outsourced
computation, but requires a trusted proxy server. Zhang24 uses
IBE-to-ABE proxy re-encryption with $2t_e$ overhead for attribute
revocation, while our version-based approach costs $<$0.1\,ms per
revocation event (one counter increment). Ciphertext size for
Sentinel-CP-ABE is $(2+2|S_T|)L_G + L_{G_T}$, comparable to BSW07's
...
$O(1)$ ($<0.06$\,ms at $N=10^5$, speedup up to $100\,378\times$).
...
101--193\,ms decryption. The closed-loop response (detection $\rightarrow$
Grad-CAM $\rightarrow$ policy update $\rightarrow$ re-encryption $\rightarrow$
re-keying) completes within $60$\,ms for 10 affected users (mean of 50
